...
def process_subject(args):
    ...
    """
    (subject_id, ses, threshold, bold_template, mask_template, masks_root_path, 
     output_dir, roi_indices, mask_type, error_log_path) = args
    ...
    connectivity_matrix = compute_functional_connectivity(subject_id, timeseries, output_dir)
...
def main(ses: str, 
    ...
    Args:
        ...
        todo_path (Path): Path to the todo file with subject IDs to be processed.
        masks_root_path (Path): Path where DK select-ROI masks are stored.
    ...
    args = [(subject, ses, threshold, bold_template, mask_template, masks_root_path, output_dir, roi_indices, mask_type, error_log_path) for subject in todo]
...
if __name__ == '__main__':
    ...
    output_directory = Path("/home/rachel/Desktop/fMRI Analysis/DK76/connectivity_matrices")
    root_directory = Path("/home/rachel/Desktop/fMRI Analysis/Scrubbed data")
    
    bold_template = os.path.join(root_directory, "{subject}", "native_T1", "{subject}_ses-{ses}_run-01_rest_bold_ap_T1-space_scrubbed_{threshold}.nii.gz")
    ...
    # main(ses=ses, threshold=threshold, todo_path=todo_file, masks_root_path=masks_root_path, output_dir=output_directory, bold_template=bold_template, mask_template=mask_template, roi_indices=roi_indices, mask_type=mask_type, multi=True)