    ...
    try:
        ...
        if mask_type == "3D":
            ...
            for i in range(atlas_img.shape[-1]):  # Iterate over the 4th dimension
                masker = NiftiLabelsMasker(labels_img=nib.Nifti1Image(atlas_img.dataobj[..., i], atlas_img.affine), standardize=True)
                timeseries = masker.fit_transform(fmri_file)
            ...
            raise ValueError(f"Unrecognized mask type {mask_type}. Should be '3D' or '4D'.")
